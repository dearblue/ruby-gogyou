...
***このブロックは struct 内部で生成されるオブジェクトが `instance_exec` するので、self が切り替わることに注意して下さい。***
...
`array_elements` は任意個の整数値で、配列型として定義することが出来ます。

`field_name` と `array_elements` は複数個並べることが出来ます。
...
クラスを MD5 のデータを保持するだけの構造から、`buffer` と `offset` を保持する構造に変更する必要があります。
その上でメソッドの定義を変更します。
